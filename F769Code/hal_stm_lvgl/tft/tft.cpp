...
void tft_init(void)
{
	...
	printf("LCD driver initialized.\n"); //debug
	// static uint8_t buf1[TFT_HOR_RES * 12]; //reduce buffer size by 8x
	// static uint8_t buf2[TFT_HOR_RES * 12];
	// static uint8_t buf1[TFT_HOR_RES * 48 *2]; //original size
	// static uint8_t buf2[TFT_HOR_RES * 48 *2];
	uint8_t *buf1 = (uint8_t *) malloc(TFT_HOR_RES * 48 * 2 * sizeof(uint8_t)); //dynamic allocation
	uint8_t *buf2 = (uint8_t *) malloc(TFT_HOR_RES * 48 * 2 * sizeof(uint8_t));
...
static void tft_flush_cb(lv_disp_t * disp, const lv_area_t * area, uint8_t * pxmap)
{
	...
	if(area->x1 < 0 || area->x1 > TFT_HOR_RES-1) return; //ignore
	if(area->x2 < 0 || area->x2 > TFT_HOR_RES-1) return; //ignore
	if(area->y1 < 0 || area->y1 > TFT_VER_RES-1) return; //ignore
	if(area->y2 < 0 || area->y2 > TFT_VER_RES-1) return; //ignore
	x1_flush = act_x1;
	y1_flush = act_y1;
	x2_flush = act_x2;
	y2_flush = act_y2;
	...
	/*Use memcpy instead of DMA for debugging*/
	// printf("x1, y1, x2, y2=%d, %d, %d, %d\n", x1_flush, y1_flush, x2_flush, y2_flush); //debug
	// printf("buf_to_flush-pxmap=%d\n", buf_to_flush-pxmap); //debug
	// printf("y_flush_act=%d\n", y_flush_act); //debug
	// memcpy(&my_fb[y_flush_act * TFT_HOR_RES + x1_flush], buf_to_flush, (x2_flush - x1_flush + 1)*2);
	//Use loop to debug
	uint8_t * temp_fb=(uint8_t*) &my_fb[y_flush_act * TFT_HOR_RES + x1_flush];
	uint8_t * temp_buf=(uint8_t*) buf_to_flush;
	for(int i=0; i<(x2_flush - x1_flush + 1)*2; i++){
		temp_fb[i]=temp_buf[i];
	}
	y_flush_act ++;
	while(y_flush_act <= y2_flush) {
	  buf_to_flush += (x2_flush - x1_flush + 1) * 2;
	//   printf("x1, y1, x2, y2=%d, %d, %d, %d\n", x1_flush, y1_flush, x2_flush, y2_flush); //debug
	//   printf("buf_to_flush-pixmap=%d\n", buf_to_flush-pxmap); //debug
	//   printf("y_flush_act=%d\n", y_flush_act); //debug
	//   memcpy(&my_fb[y_flush_act * TFT_HOR_RES + x1_flush], buf_to_flush, (x2_flush - x1_flush + 1)*2);
	  temp_fb=(uint8_t*) &my_fb[y_flush_act * TFT_HOR_RES + x1_flush];
	  temp_buf=(uint8_t*) buf_to_flush;
	  for(int i=0; i<(x2_flush - x1_flush + 1)*2; i++){
		  temp_fb[i]=temp_buf[i];
	  }
	  y_flush_act ++;
	...
	/*Use polling DMA instead of interrupt*/
	// printf("x1, y1, x2, y2=%d, %d, %d, %d\n", x1_flush, y1_flush, x2_flush, y2_flush); //debug
	// HAL_DMA_Start(&DmaHandle, (uint32_t)buf_to_flush, (uint32_t) &my_fb[y_flush_act * TFT_HOR_RES + x1_flush], (x2_flush - x1_flush + 1));
	// while(HAL_DMA_PollForTransfer(&DmaHandle, HAL_DMA_FULL_TRANSFER, 100) != HAL_OK)
	// {
  	// 	__NOP();
	// }
	// y_flush_act++;
	// while(y_flush_act <= y2_flush) {
	//   buf_to_flush += (x2_flush - x1_flush + 1) * 2;
	//   memcpy(&my_fb[y_flush_act * TFT_HOR_RES + x1_flush], buf_to_flush, (x2_flush - x1_flush + 1));
	//   HAL_DMA_Start(&DmaHandle, (uint32_t) buf_to_flush, (uint32_t) &my_fb[y_flush_act * TFT_HOR_RES + x1_flush], (x2_flush - x1_flush + 1));
	//   while(HAL_DMA_PollForTransfer(&DmaHandle, HAL_DMA_FULL_TRANSFER, 100) != HAL_OK)
	//   {
  	// 	__NOP();
